...
Default device is /dev/nvme0n1.

Examples:
  sudo $0 full /dev/nvme0n1
  sudo $0 mount /dev/nvme0n1
  sudo $0 bootloader
  sudo $0 umount
USAGE
}
...
do_mount() {
  local dev="$1"
  # Handle partition naming for nvme (p1, p2) vs sd (1, 2)
  local part_prefix=""
  if [[ "$dev" =~ [0-9]$ ]]; then
    part_prefix="p"
  fi

  echo -e "${BLUE}⠋ Opening LUKS container on ${dev}${part_prefix}2...${NC}"
  if [ ! -e /dev/mapper/crypted ]; then
    cryptsetup open "${dev}${part_prefix}2" crypted
  fi

  echo -e "${BLUE}⠋ Mounting subvolumes via Disko...${NC}"
  nix run github:nix-community/disko -- \
    --mode mount \
    ./hosts/personal-laptop/disko-config.nix \
    --arg device "\"$dev\""
...
do_full() {
  ...
  nix shell nixpkgs#nixos-install-tools nixpkgs#util-linux nixpkgs#systemd --command \
    nixos-install --flake .#personal-laptop --no-root-password --no-bootloader
  ...
  echo -e "${GREEN}Reboot and select the NVMe drive in UEFI boot menu.${NC}\n"
}
